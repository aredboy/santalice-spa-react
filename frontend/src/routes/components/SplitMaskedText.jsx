...
const SplitMaskedText = forwardRef(function SplitMaskedText(
    ...
    // 1. NUEVO REF PARA GUARDAR EL ÚLTIMO ANCHO CONOCIDO
    ...
    // 2. MODIFICAMOS EL EFECTO DE RESIZE
    useEffect(() => {
        const root = rootRef.current;
        if (!root) return;
        ...
        // Esta función decide si vale la pena reconstruir
        const checkResize = () => {
            const currentWidth = window.innerWidth;
            
            // 🎯 EL FIX MÁGICO:
            // Si el ancho sigue igual (solo cambió la altura por el scroll), IGNORAMOS.
            if (Math.abs(currentWidth - lastWidthRef.current) < 1) {
                return; 
            }

            // Si el ancho cambió de verdad (rotación o desktop resize), actualizamos la referencia y reconstruimos.
        ...
        if (window.ResizeObserver) {
            resizeObserverRef.current = new ResizeObserver((entries) => {
                // El observer dispara checkResize en lugar de reconstruir directo
                checkResize();
            });
            
            resizeObserverRef.current.observe(root);
            
            // REDUJE ESTO: Observar 6 niveles hacia arriba es muy agresivo y causa loops.
            // Con observar el root suele alcanzar para cambios de ancho.
            // Si necesitas observar padres, descomenta con cuidado.
            /* let p = root.parentElement;
            let depth = 0;
            while (p && depth < 2) { // Bajé a 2 niveles por seguridad
                resizeObserverRef.current.observe(p);
                p = p.parentElement;
                depth++;
            } 
